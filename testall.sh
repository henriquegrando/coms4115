...
Check() {
    ...
    Run "$DAMPLC" "<" $1 ">" "${basename}.c" &&
    Run "$GCC" "-o" "${basename}" "${basename}.c" &&
...
CheckFail() {
    ...
    RunFail "$DAMPLC" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
...
then
    ...
    files="tests/test-*.mpl tests/fail-*.mpl"
fi
